...
发言人   21:55
The page tables doesn't actually tell us much about what the mappings are with the page table looks like. Luckily, there's a way in qmu to ask it to print the current page table, and I print CR AC, I get into the qmu monitor or console, and if I then type info-mac, it'll print the complete page table. 
页表实际上并没有告诉我们很多关于页表之间的映射是什么样子的。幸运的是，在qmu中有一种方法可以要求它打印当前页表，然后我打印CR交流，进入qmu监视器或控制台，如果我然后输入info-mac，它将打印完整的页表。
...
发言人   32:50
And the final thing that E call does, the third thing it does is jump to the instruction that stbc points to. 
E call做的最后一件事，它做的第三件事是跳转到stbc指向的指令。
